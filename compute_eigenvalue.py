...
logInfo = open("./data/output/logInfo_{}_{}.txt".format(f"{args.model_name}_{args.model_type}", args.dataset), mode="w",encoding="utf-8")
...
@torch.no_grad()
def get_generations(args, seed=1, old_sequences=None, max_num_gen_once=args.num_generations_per_prompt):
    wandb.login(key=args.wandb_key)
    # if args.lora:
    #     runs = wandb.init(project="Eigen",mode=args.wandb_mode,save_code=True,
    ...
    metrics = ["Perplexity","Energy","NormalizedEntropy","LexicalSimilarity","EigenScore","EigenValue","EigenScore-Output"]
    meters = {metric: AverageMeter() for metric in metrics}
    device = args.device
    ...
    for batch_idx, batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        while num_gens > 0:
            dict_outputs =  model.generate(input_ids, attention_mask=batch['attention_mask'].to(device),
                            num_beams=1, 
                            # 设置num_return_sequences实现batch search
                            # Enable independenty sampling
        ...
        meters["Perplexity"].update(perplexity)
        meters["Energy"].update(energy_score)
        meters["NormalizedEntropy"].update(predictive_entropy)
        meters["LexicalSimilarity"].update(lexical_similarity)
        meters["EigenScore"].update(eigenIndicator)
        meters["EigenValue"].update(eigenValue)
        meters["EigenScore-Output"].update(eigenIndicatorOutput)
        
        cur_log = {metric:meters[metric].val for metric in metrics}
        wandb.log(cur_log)
        ...
        print("\n","\n","\n", file=logInfo)
    cur_log = {"avg"+metric:meters[metric].avg for metric in metrics}
    wandb.log(cur_log)
...
def main(overwrite=False, continue_from=None, parallel:int=None):
    if continue_from:
        ...
        args.__dict__ = jload(continue_from.replace(fname, 'args'+fname.replace("_partial.pkl", ".json")))
        ...
        cache_dir = os.path.join(_settings.GENERATION_FOLDER, f'{model_name}_{args.model_type}_{args.dataset}_{args.project_ind}')
    ...
    sequences = get_generations(args, seed=args.seed, old_sequences=old_sequences)
